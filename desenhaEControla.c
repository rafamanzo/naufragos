...
void controlaBote1(BITMAP *buffer, bote *boat, double deltaT)
{
 ...
 double ang;
	...
	if( boat->ancora == '0')
	{
		...
		{
			ang = rotacaoBote(dir);
			moveBote(dir, 10,boat, deltaT);
			rotate_sprite(buffer, boat->atr.desenho, boat->atr.pos.x, boat->atr.pos.y, ang);	
		}
		else if (key[KEY_DOWN])/*freio*/
		{
			if( (modulo(boat->atr.vel)-20) > 0 )
			moveBote(dir,(-20),boat, deltaT);
	...
	{
		if (key[KEY_ENTER]) /* Nao volta com velocidade 0 */
			boat->ancora = '0';
...
void controlaBote2(BITMAP *buffer, bote *boat, double deltaT)
{
 ...
 double ang;
	...
	{
		if (key[KEY_SPACE]) /* Nao volta com velocidade 0 */
			boat->ancora = '0';
...
void imprimeEstaticos(BITMAP *buffer, lista_estaticos estat)
{
 lista_estaticos aux = estat;
	...
	{
		if( aux -> stc.tipo == 'r' )
			 blit(aux->stc.desenho, buffer, 0, 0, 0, 0, 25, 30);/*TAMANHO DO CORAL */
		else
...
void imprimeCabecalho(BITMAP *buffer, lista_botes botes)
{
	...
	textprintf_ex(buffer, font, 780, 20, makecol(255, 0, 0), -1, "%u x", botes -> prox -> bt.vidas);
	...
	textprintf_ex(buffer, font, 850, 20, makecol(255, 0, 0), -1, "%u x", botes -> prox -> bt.carga);
	draw_sprite(buffer, pessoa, 880, 15);


	/* PONTOS */
	textprintf_ex(buffer, font, 260, 20, makecol(0, 0, 255), -1, "%d", botes -> bt.pontos);
	textprintf_ex(buffer, font, 720, 20, makecol(255, 0, 0), -1, "%d", botes -> prox -> bt.pontos);
